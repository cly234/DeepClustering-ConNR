...
class BYOL(TrainTask):
    ...
    def build_options():
        ...
        parser.add_argument('--lr_factor', help='factor of lr',type=float, default=0.1)
        parser.add_argument('--predict_lr_factor', help='factor of predictor lr',type=float, default=0.1)
        parser.add_argument('--thresh_begin', help='start thresh of bd', type= float, default=0.8)
        parser.add_argument('--epoch_end', help = 'last epoch of bd', type=int, default=850)
        ...
        parser.add_argument('--neighb_k1', help='k1 in gnn reranking', default=10)
        parser.add_argument('--neighb_k2', help='k2 in gnn reranking', default=2)
        parser.add_argument('--neighb_k', help='k in neighborhood consistency', default=10)
    ...
    def train(self, inputs, indices, n_iter):
        ...
        self.byol.module.cluster_centers = self.cluster_centers
        self.byol.module.cur_epoch = self.cur_epoch
        
        if self.cur_epoch <= opt.epoch_end:
            self.byol.module.thresh = opt.thresh_begin + (1 - opt.thresh_begin) * (self.cur_epoch - opt.resume_epoch) / (opt.epoch_end - opt.resume_epoch)
        else:
            self.byol.module.thresh = 1
        ...
        with torch.autocast('cuda', enabled=opt.amp):
            contrastive_loss, q = self.byol(
                im_q, im_k, labels, indices, update_params, False, opt.v2)
        ...
        self.logger.msg([contrastive_loss, q_std, ], n_iter)
    ...
    def adjust_learning_rate(self, n_iter):
        ...
        if opt.fix_predictor_lr:
            ...
            predictor_lr = lr * opt.predict_lr_factor
        ...
        for param_group in self.optimizer.param_groups:
            if 'predictor' in param_group['name']:
                ...
                param_group['lr'] = lr * opt.lr_factor
